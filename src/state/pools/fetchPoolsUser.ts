...
// EVT pools use the native EVT token (wrapping ? unwrapping is done at the contract level)
const nonBnbPools = poolsConfig.filter((pool) => pool.stakingToken.symbol !== 'EVT')
const bnbPools = poolsConfig.filter((pool) => pool.stakingToken.symbol === 'EVT')
