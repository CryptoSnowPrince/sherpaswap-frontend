...
const PredictionConfigProviders = ({ children }) => {
  ...
  const [selectedToken, setConfig] = useState(() => {
    ...
    if ([PredictionSupportedSymbol.EVT, PredictionSupportedSymbol.CAKE].includes(initToken)) {
      return initToken
    }
  ...
  useEffect(() => {
    ...
    if ([PredictionSupportedSymbol.EVT, PredictionSupportedSymbol.CAKE].includes(upperToken)) {
