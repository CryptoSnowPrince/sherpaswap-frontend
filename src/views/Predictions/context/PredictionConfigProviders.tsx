...
const PredictionConfigProviders = ({ children }) => {
  ...
  const [selectedToken, setConfig] = useState(() => {
    ...
    if ([PredictionSupportedSymbol.EVT, PredictionSupportedSymbol.YAK].includes(initToken)) {
      return initToken
    }
  ...
  useEffect(() => {
    ...
    if ([PredictionSupportedSymbol.EVT, PredictionSupportedSymbol.YAK].includes(upperToken)) {
